...
if (-not $env:ADO_PAT) {
	...
	write-Error "Mohan test"
    exit 1
} else {
    Write-Host "Mohan ADO_PAT environment variable is set and will be used to authenticate to Azure DevOps."
    write-host "Mohan-test"
}
...
write-Host "  Mohan test PROJECT NAME : "
...
write-Host  "Mohan print the migration ID "
write-Host $MigrationID
...
$repo_count++
write-host "  Mohan todal repo's :" $repo_count
write-host " mohan :" --migration-id $RepoMigrations["nacoolplayz/ADO_to_GitHub_migration-ADO_to_GitHub_migration"]
...
write-host "Mohan  succeeded no "  $Succeeded  
